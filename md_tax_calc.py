...
def calculate_taxes(income, flat_deduction, filing_status):
    ...
    results = {
        ...
        "Total Taxes Owed": f"${total_taxes_owed:,.2f}",
...
print("\n--- Quick Reference for Standard Deduction ---")
print(f"Standard Total Taxes Owed: ${float(standard_results['Total Taxes Owed'].replace('$', '').replace(',', '')):,.2f}")
print(f"Standard Post-Tax Income: ${float(standard_results['Post-Tax Income'].replace('$', '').replace(',', '')):,.2f}")
standard_spending_power = float(standard_results['Post-Tax Income'].replace('$', '').replace(',', '')) + standard_results['Standard Deduction']
